...
use tokio;
use ibapi::client::{Builder, Mode, Host, ActiveClient};
...
impl RemoteInitializer for AccountSummaryInitializer {
    ...
    fn build(self, client: &mut ActiveClient, cancel_loop: CancelToken) -> impl Future<Output=Self::Wrap<'_>> + Send {
        async {
            let id = client.req_account_summary(&vec![
                Tag::AccountType,
                Tag::NetLiquidation,
                Tag::TotalCashValue,
                Tag::SettledCash,
...
async fn account_summary() -> anyhow::Result<()> {
    let discon = Builder::from_config_file(Mode::Paper, Host::Gateway, Some("config.toml"))?.connect(1).await?.remote(AccountSummaryInitializer).await;
...
async fn account_update() -> anyhow::Result<()> {
    let discon = Builder::from_config_file(Mode::Paper, Host::Gateway, Some("config.toml"))?.connect(2).await?.remote(AccountUpdateInitializer).await;
    tokio::time::sleep(std::time::Duration::from_secs(3)).await;
    discon.cancel();
    Ok(())
}
...
async fn positions() -> anyhow::Result<()> {
    let discon = Builder::from_config_file(Mode::Paper, Host::Gateway, Some("config.toml"))?.connect(3).await?.remote(PositionInitializer).await;
    tokio::time::sleep(std::time::Duration::from_secs(3)).await;
    discon.cancel();
    Ok(())
}
...
impl RemoteInitializer for PnlInitializer {
    ...
    fn build(self, client: &mut ActiveClient, cancel_loop: CancelToken) -> impl Future<Output=Self::Wrap<'_>> + Send {
        async {
            let id = client.req_pnl(client.get_managed_accounts().iter().next().unwrap().clone()).await.unwrap();
            tokio::time::sleep(std::time::Duration::from_secs(1)).await;
...
async fn pnl() -> anyhow::Result<()> {
    let discon = Builder::from_config_file(Mode::Paper, Host::Gateway, Some("config.toml"))?.connect(4).await.unwrap().remote(PnlInitializer).await;
    tokio::time::sleep(std::time::Duration::from_secs(3)).await;
    discon.cancel();
    Ok(())
}
