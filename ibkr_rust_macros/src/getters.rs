...
fn impl_method(struct_name: &Ident, meth_name: &Ident, return_type: &Type, vis: &syn::Visibility, can_move: bool) -> TokenStream {
    let d = format!("Get the {}'s {}.\n\n # Returns\n The {}", struct_name, meth_name, meth_name);
...
pub fn impl_make_getters(ast: &mut ItemStruct) -> TokenStream {
    ...
    let ItemStruct { ident: ref s_name, ref vis, ref fields, .. } = ast;
    let meths = fields.iter().filter_map(|f| {
